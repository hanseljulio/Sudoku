...
public class Board {
    ...
    // My solution starts here. It's here because I want to easily call or modify any function if something goes wrong, considering how most of the code here comes from the Board file.
        ...
        for (int i = 0; i < this.board.size(); i++) {
            for (int j = 0; j < this.board.get(i).size(); j++) {
                if (this.board.get(i).get(j) == null) {
                    for (int k = 1; k <= 9; k++) {
                        Board neighbor = new Board(getBoard());
                        neighbor.setCells(i, j, k);
    ...
    public void setCells (int row, int col, int num) {
        this.board.get(row).set(col, num);
    }
